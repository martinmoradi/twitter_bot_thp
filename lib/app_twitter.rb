...
def hello_word_ruby(client = login_twitter)
  client.update('Mon premier tweet en Ruby !!!!')
end


def random_picker(handles_journalists, n)
   handles_journalists.sample(n)
end


journalists = random_picker(handles_journalists, 5)


def say_hello_journalist(journalists, client = login_twitter)
  journalists.each { |journalist| client.update("#{journalist} On ne vous le dit pas assez souvent, mais merci ! Le travail que vous faîtes au quotidien est important pour notre démocratie. #bonjour_monde @the_hacking_pro") }
end

def like_bonjour_monde(num, client = login_twitter)
    client.search("bonjour_monde", result_type:"recent").take(num).collect do |tweet|
      client.favorite(tweet)
    end
end

def follow_bonjour_monde(num, client = login_twitter)
    client.search("bonjour_monde", result_type:"recent").take(num).collect do |tweet|
...
def streaming(rest_client = login_twitter)
